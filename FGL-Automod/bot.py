...
async def verify(ctx):
    community_role = discord.utils.get(ctx.message.guild.roles, name="Community")
    message = ctx.message
    mauth = message.author
    print(mauth)
    if(ctx.message.channel.id != 734869160454258886):
        return
    t = 1
    while(t < 4):
        secret = ""
        ...
        if(msg.content == secret):
            ...
            try:
                ...
                ctx.message.author.send("Captcha timed out. Please reverify to try again!")
            try:
                it = int(msg.content)
            except:
